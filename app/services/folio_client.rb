...
class FolioClient
  ...
  # TODO: Remove after updating feature tests for FOLIO
  # API compatibility shim with SymphonyClient
  ...
  def change_pickup_library(resource, _item_key, service_point)
    change_pickup_service_point(resource, service_point)
  end
  ...
  def not_needed_after(resource, _item_key, not_needed_after)
    change_pickup_service_point(resource, not_needed_after)
  end

  # @param [String] hold_id the UUID of the FOLIO hold
  # @param [Date] expiration the hold request
  def change_request_expiration(hold_id, expiration)
  ...
  def update_request(hold_id, request_data_updates)
    ...
    response = put("/circulation/requests/#{hold_id}", json: request_data)
    check_response(response, title: 'Update or cancel request',
