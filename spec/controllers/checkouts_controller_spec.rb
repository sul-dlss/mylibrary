...
RSpec.describe CheckoutsController do
  let(:mock_patron) { instance_double(Folio::Patron, checkouts: checkouts, requests: requests, key: '513a9054-5897-11ee-8c99-0242ac120002') }
  ...
  context 'with an authenticated request for group checkouts' do
    ...
    before do
      allow(mock_patron).to receive(:proxy_group).and_return(instance_double(Folio::Group, checkouts:,
                                                                                           requests:))
    end
  ...
  describe '#renew' do
    ...
    context 'with a group request' do
      it 'renews the item and redirects to checkouts_path' do
        post :renew, params: { id: '123', group: true }
    ...
    context 'when the requested item is not checked out to the patron' do
      it 'does not renew the item and sets flash messages' do
        post :renew, params: { id: 'some_made_up_item_id' }
